/**
 * Come from java.util.concurrent.ConcurrentHashMap
 */
package actorx.util;
...
/**
 * Come from java.util.concurrent.ConcurrentHashMap, change 
...
//	private static final long serialVersionUID = 7249069246763182397L;
	private static final long serialVersionUID = -6357278694928591227L;
		...
		static {
			...
			try {
				threshold = (null != altThreshold) ? Integer
						.parseInt(altThreshold) : Integer.MAX_VALUE;
	...
	static final <K, V> HashEntry<K, V> entryAt(HashEntry<K, V>[] tab, int i) {
		return (tab == null) ? null : (HashEntry<K, V>) UNSAFE
				.getObjectVolatile(tab, ((long) i << TSHIFT) + TBASE);
	}
...
//		private static final long serialVersionUID = 2249069246763182397L;
		private static final long serialVersionUID = -8092711218560998187L;
		...
		final V put(K key, int hash, V value, boolean onlyIfAbsent) {
			HashEntry<K, V> node = tryLock() ? null : scanAndLockForPut(key,
					hash, value);
			V oldValue;
			try {
	...
	static final <K, V> Segment<K, V> segmentAt(Segment<K, V>[] ss, int j) {
		...
		return ss == null ? null : (Segment<K, V>) UNSAFE.getObjectVolatile(ss,
				u);
	}
	...
	public V putIfAbsent(K key, V value) {
		...
		int hash = hash(key);
		int j = (hash >>> segmentShift) & segmentMask;
		if ((s = (Segment<K, V>) UNSAFE.getObject(segments, (j << SSHIFT)
				+ SBASE)) == null)
			s = ensureSegment(j);
	...
	private void readObject(java.io.ObjectInputStream s) throws IOException,
		...
		final Segment<K, V>[] oisSegments = (Segment<K, V>[]) oisFields.get(
				"segments", null);

		final int ssize = oisSegments.length;
		if (ssize < 1 || ssize > MAX_SEGMENTS || (ssize & (ssize - 1)) != 0) // ssize
																				// not
																				// power
																				// of
																				// two
	...
	static {
		...
		try {
			...
			HASHSEED_OFFSET = UNSAFE.objectFieldOffset(ConcurrentHashMap.class
					.getDeclaredField("hashSeed"));
			SEGSHIFT_OFFSET = UNSAFE.objectFieldOffset(ConcurrentHashMap.class
					.getDeclaredField("segmentShift"));
			SEGMASK_OFFSET = UNSAFE.objectFieldOffset(ConcurrentHashMap.class
					.getDeclaredField("segmentMask"));
			SEGMENTS_OFFSET = UNSAFE.objectFieldOffset(ConcurrentHashMap.class
