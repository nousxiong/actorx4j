...
//		final String CLN_ADDRESS = "tcp://127.0.0.1:23334";
		...
		ActorId srvAid = srvAxs.spawn(srvBaseAx, "SRV", new IThreadActorHandler() {
			@Override
		...
		ActorId svcAid = ServiceUtils.makeServiceId("AXSSRV", "SRV");
		assertTrue(ActorId.equals(srvAid, svcAid));
		
		clnBaseAx.monitor(svcAid);
		...
		clnBaseAx.send(svcAid, "INIT");
		final int pipeline = 1000;
		for (int i=0; i<pipeline; ++i){
//			clnBaseAx.send(srvAid, "HELLO", "Hello World!");
			clnBaseAx.csend(svcAid, new Hello("Hello World!"));
//			Strand.sleep(3000);
		}
		for (int i=0; i<pipeline; ++i){
			clnBaseAx.recv(cmsg, "HI");
		}
		clnBaseAx.send(svcAid, "HELLO", "end");
		
		Message msg = clnBaseAx.recv(cmsg, "OK");
		assertTrue(ActorId.equals(msg.getSender(), svcAid));
